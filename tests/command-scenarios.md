...
| R10 | `start`; saved auto-deploy=true, batch=2 | Display wordmark, show the effective saved choice, and proceed without another confirmation. Preserve release gates; an explicit `no-auto-deploy` would override it for this run. |
...
| R30 | Saved harness=`claude-code`, custom Claude `role_routing`, custom `run_defaults`; trusted runtime metadata identifies Codex | Before settings/start/dispatch, save harness=`codex`, remove only `role_routing`, preserve `run_defaults` and unrelated receipt fields, report the switch, and use Codex adapter defaults without asking. |
| R31 | Bare `settings`; valid saved values and all Codex roles are available | Show the wordmark once. Ask one numbered question at a time for all four run settings, then each role's model followed by effort. Support Keep current, Back, and Cancel. Review once and write atomically only after Save. |
| R32 | Bare `setup`; Beads and LeanCTX are missing | Show each missing dependency's inspected plan one at a time and offer numbered Install now, alternate supported scope, Skip and remember, or Cancel choices. Never infer consent across items. If setup continues, run the complete settings wizard. |
