...
function laneRows(lanes) {
  return lanes.map((lane) => `<tr><th scope="row">${escapeHtml(lane.id)}</th><td>${escapeHtml(lane.teamId)}</td><td>${escapeHtml(lane.role)}</td><td>${escapeHtml(lane.status)}</td><td>${escapeHtml(lane.currentTaskId || "Idle")}</td><td>${escapeHtml(lane.queue?.join(", ") || "None")}</td><td>${escapeHtml(lane.remainingTaskIds?.join(", ") || "None")}</td><td>${escapeHtml(lane.rotationCount)}</td><td>${escapeHtml(lane.liveness)}</td></tr>`).join("");
}
...
${model.lanes?.length ? `<section id="lanes" aria-labelledby="lanes-title"><h2 id="lanes-title">Lane capacity</h2><p>Logical lanes: ${escapeHtml(model.slots?.logical?.occupied ?? "unknown")} occupied of ${escapeHtml(model.slots?.logical?.limit ?? "unknown")}. Native capacity: ${escapeHtml(model.slots?.native?.occupied ?? "unknown")} occupied of ${escapeHtml(model.slots?.native?.limit ?? "unknown")}; ${escapeHtml(model.slots?.native?.reservedReview ?? "unknown")} reserved review; ${escapeHtml(model.slots?.native?.unknown ?? "unknown")} unknown.</p><div class="table-wrap" tabindex="0"><table><thead><tr><th>Lane</th><th>Team</th><th>Role</th><th>Status</th><th>Current task</th><th>Planned queue</th><th>Remaining</th><th>Rotations</th><th>Liveness</th></tr></thead><tbody>${laneRows(model.lanes)}</tbody></table></div></section>` : ""}
