...
After an accepted integration and its normal Beads closure, ask a lowest-cost model actually available on the host to refresh `.agent-team/dashboard/index.html` from `assets/dashboard.html`. It is a local, static status snapshot, not a service or task authority. Run every Beads read for the refresh with `bd -C <target-project-root> ...`, including aggregate status and the normal 20-row `ready --limit 20 --json` page, so all data comes from the exact target project even when Beads holds 1,000 or more tasks. Before writing the snapshot, verify the just-closed issue in that same database with `bd --readonly -C <target-project-root> show <closed-ID> --json` and confirm the returned ID matches `<closed-ID>`. If the lookup fails or the ID differs, stop the refresh and report a dashboard-only failure; never reopen, block, or otherwise alter the accepted Beads issue. Do not forward issue rows to a model. Mark the snapshot time and source. For 1–20 ready rows, populate the table body, unhide the table, and hide the empty copy; for none, keep the table hidden and show the empty copy. Report any refresh failure as dashboard-only; it never changes the accepted Beads issue.
