...
Evidence status: candidate CI passed the POSIX and PowerShell installer checks; native Windows CI run `36106466674` passed at revision `ea749d9`, closing installer test Bead `atv-uns.7`. This is installer fixture evidence only, not a v8 managed-uninstall result. After the separately authorized Linux force-removal cutover, installed Codex and Claude v9 status commands returned Beads data without setup or dispatch. These are status-only canaries, not native-worker acceptance. No Windows or managed-v8-uninstall cutover pass is asserted here; the destructive live route has no rollback claim. See [the release ledger](../../docs/releases/9.0.0-readiness.md).
