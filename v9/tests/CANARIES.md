...
- After each successful integration, clean merged task worktrees and branches are removed without force. Exercise a clean task branch integrated by cherry-pick too: remove it only after recorded verified patch-equivalence identifies its exact task scope and exact integrated revision. A dirty, unverified, unmerged, unknown, or failed task worktree/branch remains and is reported.
