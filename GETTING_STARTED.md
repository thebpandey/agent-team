...
The first-use workflow below describes the current source. Published download versions remain those identified in the [README](README.md); these changes do not announce a new release. Use a controller built from the matching source when exercising the expanded setup and Claude dispatch flow.
...
| `approve_kickoff` | Review the discovered nested Project Kickoff 0.5.0 handoff and approve using its existing facts. |
