...
Agent-Team 7.2.3 uses one complete package for Codex and Claude Code. Installation lets you choose the host and whether to install for one project or your user account. The [standalone dark-green HTML guide](Getting_Started_with_Agent-Team.html) remains a historical guide; the [README](README.md) is the current release and workflow reference.
...
Download `agent-team-7.2.3.zip` and its matching one-entry `SHA256SUMS` from [v7.2.3](https://github.com/thebpandey/agent-team/releases/tag/v7.2.3), with update discovery at [releases/latest](https://github.com/thebpandey/agent-team/releases/latest). Verify and install the sealed bytes:
