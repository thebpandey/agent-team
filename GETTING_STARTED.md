...
Do not overwrite a v7 project or top-level skill manually. The current v7 state/receipt chain cannot immutably authorize the different Beads scope. A Unix operator must provision `/etc/agent-team/cutover-trust.json` out of band as a root-owned, non-group/world-writable schema-1 document: `{"schema":1,"keys":[{"algorithm":"ed25519","keyId":"<sha256-of-raw-key>","publicKey":"<base64-raw-key>"}]}`; multiple keys are sorted uniquely by key ID. This package has no API that writes the trust store. Sign the canonical JSON encoding of a schema-1 approval with `signature` empty. It must bind its ID, issue/expiry times (at most 24 hours), absolute project, operation ID, target revision, tracker fingerprint/parent/task IDs, sorted unique review/test/readiness IDs plus absolute paths and SHA-256 digests, cause, recovery disposition, signer key ID, configured remote name/URL/refs/revisions/absence, and `remoteMainDeploys`. Put only the signed file's ID, absolute path, and SHA-256 in request `approval`. Windows cutover currently refuses because system trust-store ACL ownership cannot yet be proven. Then run `agent-teamctl cutover --request /absolute/request.json`. A project request uses `action: "cutover"`, then `status`, an approval- and receipt-bound `reconcile`, or an exact `rollback`. A host request uses `host-cutover`, `host-status`, or `host-rollback`, the exact schema-4 legacy installer receipt and digest, the current native manifest revision, and `hosts: ["codex", "claude"]` as applicable. Keep the request and returned receipt digest: rollback refuses a different receipt, revision, changed legacy file, or customized hook.
