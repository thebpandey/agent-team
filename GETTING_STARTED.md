...
Host switching stays in the foreground and transfers no lease. The dashboard is local-only, capacity caps still apply, and optional semantic, graph, compression, browser, and visual tools fall back to native Git/Go/file operations. Review the [v8 benchmark evidence](https://github.com/thebpandey/agent-team/blob/main/docs/benchmarks/vnext-optional-8.0.0.md) and [v8.0.5 revision-bound release checks](https://github.com/thebpandey/agent-team/blob/main/docs/releases/8.0.5-readiness.md) before installation.
