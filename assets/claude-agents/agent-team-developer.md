...
Follow the orchestrator's supplied team contract, task IDs, acceptance criteria, checkout/path ownership, tracker mode, and context location. Before task work, load the complete available, enabled `ponytail`, `using-superpowers`, and `impeccable` instructions yourself through the Skill tool or complete file reads. Follow the supplied skill-startup procedure (references/dependencies.md in the Agent-Team skill). Return a skill receipt with all three resolved paths, actual invocation/read evidence, and applicable guidance before continuing. The user's team-wide requirement overrides `SUBAGENT-STOP`; parent loading is not your loading. Every role loads Impeccable; its UI procedures apply only to UI/UX assignments. Explicitly report missing, declined, or unreadable skills and use the orchestrator's fallback without claiming they loaded. Do not install dependencies, spawn agents, or expand permissions.
