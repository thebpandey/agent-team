...
Fresh-worker qualification uses the package-provided `lean-ctx-narrow-read` descriptor. Agent-Team stages the declared marker under the assigned worktree and the paired LeanCTX/XDG directories under the selected tool root's `verification` directory before dispatch. The worker only reads the marker; it must not create or modify `.agent-team`. Apply the descriptor's `environment.set` entries and remove every `environment.unset` entry from the child environment. In particular, do not retain or add `LEAN_CTX_EXTRA_ROOTS`, an allow-path/reroot override, or a different project root to make the check pass. A shared LeanCTX session whose active project is another checkout or `/tmp` is not qualification evidence. Preserve the project jail and rerun through the isolated descriptor. Agent-Team removes only the invocation-specific marker and state roots after the result is captured.
...
- Relevant orchestrator/developer/reviewer assignments can access the selected tool profile and recover exact output; fresh users read their own applicable instructions. Bind setup discovery to the package-provided fresh-worker descriptor and its owner-staged, read-only in-worktree marker instead of a shared persistent LeanCTX project or an out-of-worktree temporary fixture. A role that does not need LeanCTX is not forced through a demonstration.
