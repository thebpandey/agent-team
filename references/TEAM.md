...
For retained multi-task work, follow [lanes](LANES.md). One lane binds one brief, ordered task packets, worker generations, handovers, worktree, branch, and immutable assignment/result history. The tracker still owns task status, and lane records never authorize work outside the assigned queue and paths.
...
Delegate the checks that inform a packet before dispatch and the final checks after a team reports completion. Use the saved reviewer or visual reviewer profile through the actual host; current missing-role defaults are `gpt-6-sol` for Codex and `claude-opus-5-5` for Claude. Explicit inheritance and custom choices remain authoritative. A cross-host plugin route requires an approved, available selection; it never silently overrides the saved profile. The verifier reports; the owning developer repairs; the orchestrator integrates the accepted revision.
