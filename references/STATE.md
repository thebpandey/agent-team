...
[Lanes](LANES.md) add durable queue, assignment, rotation, packet, and handover projections to canonical state. The selected tracker remains the only task authority: a lane queue orders already authorized work but never claims a future task or creates another status ledger.
...
Use [project coordination](PROJECTS.md) for stable team IDs, serialized shared-record writes, and one canonical tracker across worktrees. TEAMS.md holds identities and resource locations only. Team leads route updates to the active project orchestrator where direct writes are not supported.
...
Keep immutable initialization (`initialTaskIds`, tracker fingerprint, consumed handoff and original scope) separate from versioned effective-run transitions. Supported workflow names are exactly `run-start`, `run-reconcile`, `run-scope-extend`, `completion-quarantine`, `completion-rebind`, `evidence-store-register`, `completion-history-reconcile`, and read-only `run-decision`. Scope extension is native, versioned, and additive. Completion, integration, and publication evidence remains task-keyed across current and historical records.
