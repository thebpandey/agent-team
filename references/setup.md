...
1. **Required + project needs (Recommended):** install missing Ponytail, Using-Superpowers, and Beads; include Impeccable for UI work and only optional tools relevant to this project.
...
3. **Choose individually or defer:** let the user select items or continue without installation; missing mandatory items block only the affected implementation, optional items do not.
...
| --- | --- | --- |
| Ponytail | Required skill/plugin: https://github.com/DietrichGebert/ponytail | Resolve `ponytail`; inspect plugin hooks and use the supported host skill/plugin installer |
| Using-Superpowers | Required skill in https://github.com/obra/superpowers | Install the supported distribution and referenced companion resources needed by this skill; resolve `using-superpowers`; do not pretend one copied file provides the framework |
| Beads | Required tracker: https://github.com/gastownhall/beads | Install the supported `bd` release for the OS; initialize project tracking only after checking existing configuration and concurrent-write support |
| Impeccable | UI-required skill/tool: https://github.com/pbakaus/impeccable | Install the correct provider build and required engine/resources; respect hook trust prompts |
...
After installation, verify each selected skill resolves by name and supporting files exist; verify relevant CLIs run their version/help checks and packages resolve in the intended project. Recheck required skill visibility before spawning teammates. Use a targeted smoke check when a dependency's behavior or native binary requires it. Reuse unchanged evidence; do not launch the full application test suite solely because documentation skills were added.

Record installed/verified, failed, deferred, incompatible, and excluded items accurately in the setup receipt. If Beads is ready, link meaningful setup failures and the receipt from the task; before it exists the receipt is a bootstrap record, not an alternative tracker. Avoid repeated install loops. Preserve successful installs when another fails. If the user declines a mandatory item, stop affected implementation and explain the exact blocker; continue independent authorized work. Optional installation failure never blocks an otherwise valid task.

End setup with a short status and resume the original task automatically when required dependencies are usable. A setup-only request ends after the status. Later runs recheck presence and changed environment fingerprints, retain the user's profile and applicable authorization, and ask only for newly proposed scope. Do not upgrade dependencies on every invocation.
