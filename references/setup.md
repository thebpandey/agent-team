...
During explicit setup for an active project, the canonical project owner uses the normal host session so `SessionStart` can create or update `.agent-team/operation-mappings.json` after healthy state validation. Do not accept a caller-provided owner or session flag. Successful owner state writes also refresh this locked policy cache. Report missing, invalid, or stale cache health as unavailable fallback evidence where applicable. Read-only checks and status actions do not create or refresh the cache. The cache stores validated operation mappings, not tasks or progress.
