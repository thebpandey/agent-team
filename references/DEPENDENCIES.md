...
| --- | --- | --- |
| [Serena](https://github.com/oraios/serena) | Default | Semantic navigation with isolated active project/worktree context. Direct MCP qualification and fresh-worker inheritance are separate observations. |
| [Microsoft Playwright CLI](https://github.com/microsoft/playwright-cli) | Default | Browser CLI for tasks that need it. Qualify its exact executable; Node REPL imports and native browser adapters are separate capabilities. |
...
Compatible existing installations are reused. Prepare selected default capabilities on first run without individual plugin approval questions; optional components remain off until selected. Inspect and pin sources before installation, preserve customizations, and respect actual auth/trust/admin boundaries. A selected companion's failure is diagnostic unless the active plan explicitly lists it in `requiredCapabilities`.

Compatibility and package lifecycle management are separate. When every catalog-required file/digest and unique entrypoint qualifies, functional and fresh-worker probes pass at the exact selected path, and no managed sidecar exists, record `status: "passed"`, `installed: "reused_unowned"`, and the compatibility field `lifecycleOwnership: "unowned"`. That field describes install/uninstall responsibility only; it never grants or restricts session coordination. Make no sidecar, copy, overwrite, rollback entry, or uninstall authority; probes remain bound to that exact path. Unrelated regular files outside the catalog-required surface are allowed and remain byte-identical. A changed/missing required file, symlink/special entry, ambiguous entrypoint, or incompatible identity is `manual_action`/`cannot_use` with exact component/path metadata and zero write.

Prerequisites are exact catalog edges, not grouped-product similarity. LeanCTX does not depend on Graphify and Graphify does not depend on LeanCTX; one failure cannot poison the other's independently qualified receipt. Preserve each selected path and component's own compatibility and package-lifecycle evidence.
