...
| 2. Project and path boundary | Enforce | Require trusted native Codex/Claude runtime metadata in the same canonical Git project. Permit every path inside the active checkout regardless of recorded coordinator or team identity; still deny Git metadata and resolved paths outside that checkout. |
| 3. Integration and release | Enforce | Keep integration and release separate. Integration preserves its configured `baseRef`, records the observed remote main base separately as exactly `refs/heads/<baseRef>`, binds an exact HEAD and bounded authorization provenance, derives clean delta and whether the exact target updates main, requires reconciled recovery, preview, and explicit main-deployment evidence, rechecks that remote is unchanged, and permits only one structurally parsed non-force `HEAD:target` advance whose observed base is an ancestor of exact HEAD. An exact valid `refs/tags/<tag>` target may be created only with explicit absent-target evidence and an immediate absence re-probe. Pull-request integration remains a separate supported path. Release binds authorization provenance, process, run/batch, task IDs, artifact/revision, integration, verification, preview, pause/hold, delta, and recovery records. Historical session fields are provenance, not current-session authority. |
...
| 10. Completion | Enforce | Gate explicit final transitions against canonical task identity/scope, revision, checks, review, requirement evidence, and deployment/cleanup evidence only when each flag is in scope. Permit factual partial, blocked, and deferred outcomes. Ordinary `Stop`, status, questions, and interruption events do not complete a task. |
...
The 7.2.6 lowercase-to-uppercase Markdown migration follows that manual boundary. Before moving anything, compare every managed package and lowercase Claude role against the schema-4 receipt's exact byte, mode, and size evidence; customized or incomplete state is a conflict and stays untouched. With separate replacement authorization and all writers stopped, retain rollback backups of both selected package targets and every unchanged managed lowercase role, then run the normal fresh absent-target installer. Never leave the lowercase and uppercase filename for one native role ID active together. If fresh installation fails, remove only the proven new outputs and restore the retained preimages. The installer intentionally does not infer this replacement authority from version numbers or ownership receipts.
...
| --- | --- |
| `project-initialize --project /path/to/project --request /path/to/request.json` | Create/adopt approved canonical records with exact project, branch and complete tracker IDs. Reports canonical readiness separately from companion observations. |
| `project-owner-recover`, `coordinator-continuity-transfer`, `legacy-owner-adopt` | Deprecated compatibility/maintenance routes. Never use them for ordinary Claude↔Codex or fresh-session continuity; current operations do not depend on their records or journals. |
...
| `dependencies`, `dependencies-prepare`, `readiness` | Require project/host and selected user/project capability scope. Installation is not proof of functionality; an unobserved worker is `unknown`, and only explicit plan requirements gate dispatch. |
| `helpers`, `helpers-install`, `helpers-recover` | Inspect or install the three managed project scripts under `scripts/agent-team`. Native mutations record exact hashes through the canonical setup writer; customized files and interrupted transactions remain visible. |
| `context-reduction`, `context-reduction-apply`, `context-reduction-revert`, `context-reduction-recover` | Offer, apply, revert, or recover a project-level MCP/plugin reduction. Missing automatic inventory is `visibility_unknown`; an acknowledged report stays unverified. Native project identity, canonical dependency exclusions, exact proposal/receipt binding, and trusted adapter paths are mandatory for writes. |
| `dashboard-configure` | Save explicit snapshot/optional graph preferences under setup version checks; never starts a server or installs bv. |
...
| `task-transition --project /path/to/project --request /path/to/request.json` | Claim, pause, park or resume through canonical version/project checks. Does not terminate or spawn agents. |
| `run-start`, `completion-quarantine`, `completion-rebind`, `evidence-store-register`, `completion-history-reconcile` | Apply only their exact versioned workflow transition with task-keyed provenance. |
| `run-reconcile`, `run-scope-extend` | Apply the exact versioned transition only when the installed native `PreToolUse` adapter observes the exact non-chained CLI command and derives its project/session identity. The exported `runCommand` router refuses caller-supplied identity for these routes; scope extension remains additive. |
...
| `gate-evidence --project /path/to/project --request /path/to/request.json` | Bind a passed evidence artifact to the current exact revision and task set. Completion requires reconciled requirements, independent review, and named passed checks. Integration requires the exact remote/base/target observations, recovery and preview disposition, target authorization, clean lineage, and explicit `remoteMainDeploys`. Recorded session fields bind evidence provenance but never require the current native session to equal a historical coordinator. Accepted integration queues only completed top-level in-run deliveries. The helper does not run tests or grant caller-supplied authority. |
...
The legacy owner helpers remain packaged only for compatibility and explicit historical-record maintenance. They are never prerequisites for normal work, and their journals/history do not fence setup or operational mutations. Host/session switching follows [session continuity](ACTIONS.md#continue-in-another-host-or-session) and creates no takeover file.

### Release evidence artifact

`gate-evidence` accepts a release artifact only through the same versioned native-project request envelope used by the other operational mutations. The artifact is JSON with `status: "passed"`, the exact clean `revision` and release `taskIds`, and these release fields:

- the provenance `ownerSessionId`, `authorized: true`, matching `expectedRevision`, bounded `target`, and release `process`;
- `authorization` with a nonblank source of at most 256 characters, the same target/process, `scope` equal to `batchId`, matching internal provenance, and a parseable `grantedAt` value;
...
The helper rechecks the current tracker, clean HEAD, internally consistent release/integration evidence, the prior integration receipt, project pause, operational version, and task identities while holding the state lock. It rejects incomplete or inconsistent release evidence as `release_evidence_mismatch`. On acceptance it maps only validated fields and records evidence paths/digests. Release policy then revalidates the run record and artifact/checksum metadata before allowing the named process. This records existing batch-scoped authorization; it cannot create broader or future-batch authority.
...
{
  "schemaVersion": 1,
  "actorSessionId": "current-native-session",
  "expectedVersion": 4,
  "request": {
...
Replace every example value with observed facts. `status.versions.operational` and `status.freshness.fingerprint` supply transition preconditions; `task.canonicalOwner` preserves Beads' empty unassigned assignment, unlike the human display label. Never guess a zero version after an unavailable read. Re-read on conflict; do not reuse an operation ID with different intent. JSON identity fields are not authentication. For `run-reconcile` and `run-scope-extend`, only the installed native `PreToolUse` adapter derives host, session, and canonical cwd from the observed event; the exported `runCommand` router rejects a caller-supplied substitute. A request, CLI `--project`, shell subprocess cwd, syntax-valid envelope, or direct Node call cannot assert or mint that runtime identity.
