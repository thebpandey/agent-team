...
| `legacy-owner-adopt --project /absolute/project --request /absolute/request.json` | One-time migration for the historical synthetic `root` owner or self-qualification of an otherwise exact legacy owner that already equals the trusted native session. The bare CLI validates intent or reads an exact permanent replay receipt; only the installed native `PreToolUse` adapter may apply it. |
...
| `helpers`, `helpers-install`, `helpers-recover` | Inspect or install the three managed project scripts under `scripts/agent-team`. Native owner mutations record exact hashes through the canonical setup writer; customized files and interrupted transactions remain visible. |
| `context-reduction`, `context-reduction-apply`, `context-reduction-revert`, `context-reduction-recover` | Offer, apply, revert, or recover a project-level MCP/plugin reduction. Missing automatic inventory is `visibility_unknown`; an acknowledged owner report stays `owner_reported_unverified`. Native owner identity, canonical dependency exclusions, exact proposal/receipt binding, and trusted adapter paths are mandatory for writes. |
...
| `run-start`, `completion-quarantine`, `completion-rebind`, `evidence-store-register`, `completion-history-reconcile` | Apply only their exact owner/versioned workflow transition with task-keyed provenance. |
| `run-reconcile`, `run-scope-extend` | Apply the exact owner/versioned transition only when the installed native `PreToolUse` adapter observes the exact non-chained CLI command and derives its identity. The exported `runCommand` router refuses caller-supplied identity for these routes; scope extension remains additive. |
...
| `gate-evidence --project /path/to/project --request /path/to/request.json` | Bind a passed evidence artifact to the current exact revision and task set. A `completion` artifact names exactly one task and includes `requirementsReconciled: true`, a passed `{ status, revision, taskId }` review, and one or more passed `{ name, status, revision, taskId }` checks, all for that task and revision; the helper maps only those validated fields into `state.completion` with provenance. An `integration` artifact must include exact remote name/base-ref/base-revision/target-ref evidence, with its base ref exactly `refs/heads/<configured baseRef>`, reconciled `{ status, revision, taskIds }` recovery evidence, applicable preview evidence, explicit boolean `remoteMainDeploys`, and `{ source, scope: "integration", ownerSessionId, revision, taskIds }` authorization provenance. Existing targets require `targetRevision`; a new valid tag target requires `targetAbsent: true` and omits `targetRevision`. The owner-only mutation derives clean delta and main targeting, records its own observation time, preserves registered owner/configured `baseRef`/pause/ordinary holds, and policy revalidates the exact structurally parsed non-force advance (including an immediate absent-tag re-probe and canonical base-ref binding). Accepted integration evidence queues completed top-level, nondeployed delivery tasks in integration order, including recovered completions already scoped into the run; any incomplete top-level delivery makes the integration fail closed, while subtasks and epics are never queued. An initially unauthorized adoption gate remains held when its first fresh evidence operation records authority. A second fresh owner-authorized evidence operation clears that adoption hold only when the exact prior integration record/operation receipt and durable adoption receipt still bind the current epoch-one owner and empty epoch-one history; absent, malformed, stale, or mismatched proof preserves the hold. Does not run tests or grant caller-supplied authority. |
...
`legacy-owner-adopt` is not owner recovery and performs no liveness inference. Its closed schema accepts the expected project/revision/tracker and record fingerprints, an explicit `legacy_owner_adoption` authorization, and a reason; actor, replacement owner, writer, liveness, and capability fields are forbidden. The adapter derives the new runtime/session/canonical cwd from the native event and captures the `/proc` writer identity. It accepts only the exact, non-chained installed CLI form on canonical clean `main`, no pending shared operation, and either the unqualified synthetic `root` shape or an unqualified legacy owner whose request, project/integration records, exact `TEAMS.md` labels, and trusted native session all carry the same session ID. A different legacy UUID, host, cwd, or caller-supplied replacement cannot qualify. The transaction publishes epoch-one ownership plus an empty epoch-one history through a dedicated durable journal without changing run scope. Existing integration/release evidence is retained but made inert with `authorized: false` and `hold: true`. Exact replay is idempotent; another adoption, altered replay, partial/tampered journal, linked worktree, stale hash, or manual ambiguity fails closed. After adoption, the same exact native route may apply `gate-evidence`; the first fresh integration operation records authority while retaining the hold, and the next fresh operation can remove it only while that prior integration receipt and the permanent adoption receipt exactly match the current epoch-one owner. A plain CLI still cannot mint mutation authority.
...
Replace every example value with observed facts. `status.versions.operational` and `status.freshness.fingerprint` supply transition preconditions; `task.canonicalOwner` preserves Beads' empty unassigned owner, unlike the human display label. Never guess a zero version after an unavailable read. Re-read on conflict; do not reuse an operation ID with different intent. JSON identity fields are not authentication and cannot override the registered canonical owner. For `run-reconcile` and `run-scope-extend`, only the installed native `PreToolUse` adapter derives host, session, canonical cwd, and ownership epoch from the observed event; the exported `runCommand` router rejects a caller-supplied substitute. A request, CLI `--project`, shell subprocess cwd, syntax-valid envelope, or direct Node call cannot assert or mint that identity.
