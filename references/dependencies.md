...
1. Resolve `ponytail`, `using-superpowers`, and `impeccable` to their installed names and absolute SKILL.md paths (or exact resource identifiers). The dispatch includes all three, their enabled/missing/declined states, and the absolute path to this procedure. Discovery alone is not loading. Explicit user instructions to use the skills take precedence over dependency defaults and Using-Superpowers' `SUBAGENT-STOP` exemption.
...
3. Apply the loaded instructions within the assignment. Every role loads all three; Impeccable's UI setup, commands, and visual checks apply to UI/UX work only. Non-UI agents record it as loaded with no UI action applicable. Respect role limits and the user's proportional workflow; a dependency cannot authorize new tasks or agents.
4. Return a compact **skill receipt** as the first task update: agent/session identity, then one row per skill containing its resolved name/path, `loaded` / `missing` / `declined` / `unreadable`, the actual invocation or read evidence, and one applicable rule (or why its domain-specific actions do not apply). Store this receipt with the agent's existing CONTEXT.md checkpoint; do not create another ledger. Never fabricate a tool event or mark a failed read as loaded.
5. The dispatching orchestrator checks each receipt against the supplied three-skill list and available read/invocation evidence before accepting task work or permitting further dispatch. If a receipt omits an enabled skill or only repeats its name, send the agent back through startup before it continues. Project Orchestrators publish their own receipt before dispatch; Team Orchestrators report theirs to the project owner before dispatching teammates. Missing, declined, or unreadable dependencies use the existing disclosed fallback and local tracking; report incomplete skill loading without blocking otherwise feasible work or claiming all three ran. Do not install or prompt independently.
