...
Offer installation of missing Beads and the following recommended skills. Use enabled, available skills for Astra and every teammate, including reviewers and testers; missing or declined dependencies do not block work.
...
Use `$skill-name` in Codex CLI/IDE; use the host's skill selector or `@skill-name` where supported in ChatGPT. These references are skill names, not universal shell commands. Plugin names and their skills may differ. A reference does not guarantee installation, execution, model selection, or hooks.
