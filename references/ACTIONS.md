...
Native `lane-create`, `lane-next`, `lane-rotate`, and `lane-close` follow the closed request phases in [lanes](LANES.md). Natural-language intent selects a workflow; it does not supply native identity, skip current fingerprints/revisions, or expand tracker and path authority.
