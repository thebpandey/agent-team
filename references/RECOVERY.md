...
Recover [lanes](LANES.md) by their exact lane, worker generation, worktree, current claim, packet/handover hashes, and pending dispatch result. Unknown liveness remains occupied and is never stopped-writer proof; rotation preserves the current claim until an explicit transfer or authenticated stop observation permits rebinding.

## Switch coordinator between Codex and Claude Code

When the user asks the current native session to take over or continue, follow [session continuity](ACTIONS.md#continue-in-another-host-or-session): verify the same canonical Git project and proceed. Do not stage a request file, require the prior coordinator, compare session UUIDs or epochs, or invoke an owner recovery/migration helper.

The switch preserves tracker state, task claims, active-run choices, settings, checkpoints, pending operations, and evidence. It does not prove old worker liveness; unknown writers remain occupied. Consequential release records still need their exact evidence, target, revision, task scope, and explicit authorization, but the recorded coordinator session is not an additional gate. Ask about an unsupported model fallback separately.

Restore [run scheduling and batches](RUNS.md) along with team state. Preserve effective run choices, explicit run overrides, fixed membership or continuous scope, occupied slots, integration boundaries, pending/in-flight batches, and deployment-failure holds. Saved defaults changed after start do not change the resumed run. Reconcile the current harness before replacement dispatch, but do not replace recorded run choices. Any verified native project session may restart admission or submit exact release evidence; a replacement worker still cannot overwrite an active writer's paths.
...
Retirement requires current state, run, tracker, and Git revision with no other pending operation. It archives the historical run and gate snapshots, clears only the active-run pointer, and holds integration and release until fresh evidence is recorded. It does not close tasks, release claims, alter worktrees, treat writers as stopped, or discard evidence. The next `run-start` must come from a native Claude Code or Codex session in the same Git project and exactly match the pending successor run ID and sole task ID with explicit settings and auto-deploy off; ordinary gates must establish fresh integration and release evidence afterward.
...
The active project orchestrator records pause intent for the targeted teams through the canonical tracker before requesting safe checkpoints. Stop new assignments, integrations, and releases for those teams. Include the project orchestrator's active integration or release work in a project-wide pause. Finish only the minimum observation or safe recovery needed for an operation that cannot safely stop immediately; do not begin another release or delete worktrees as part of pause.

Request pauses from the affected team leads through supported controls; each lead applies the procedure below to its members. A lead without cross-session control sends the active project orchestrator a request or leaves a durable handoff. Report delivery or access limits, not a successful global pause. Keep the fan-out bounded: do not wait indefinitely on an unavailable session or repeatedly send the same request.
...
3. Inspect saved files and uncommitted changes, current branches, worktree ownership, process/session identities, preview state, evidence revisions, and in-flight provider operations through supported read access. Check whether original writers are alive. A lost connection, reused PID, or stale timestamp does not prove they stopped. Local writer checks also require the PID namespace in which the identity was recorded: a sandbox can hide a live host process. Missing namespace metadata, including older records, or a different namespace means unknown activity. Preserve the gates until supported observation from the matching host process view resolves the identity.
...
5. Reconcile interrupted operations from external evidence before retrying. If deployment succeeded, have the delegated verifier confirm it and record it without deploying again. If cleanup stopped midway, inspect each recorded resource and remove only what remains eligible. Inspect unresolved Git operations without resetting them. Unknown outcomes stay unknown until resolved.
6. Restore the preview gate, pending approval, reviewed version, and release authority. Resume never implies approval. Restart a dead preview only under its existing authorization after checking port and process ownership, then have the delegated verifier confirm its served version. Do not call an old URL live without that confirmation.
...
Select checkpoints by project/task/session/worktree and relevant revision, not simply the newest file in the project. After restoration the orchestrator validates ownership, revision pointers and pending operations through the recovery helper, and has the delegated verifier validate current source and evidence. A newer checkpoint for another task is not this task's memory. Preserve approved/rejected choices, scope exclusions, failed approaches and pending-operation uncertainty.
