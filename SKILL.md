...
Read [state and recovery](references/state.md) when setting up tasks, handling failures, or checkpointing. The active tracker owns task status and failure history. Project docs own durable architecture and design decisions. Each agent's local `CONTEXT.md` owns only resumption notes and pointers.
