---
name: agent-team
description: Coordinate software and app implementation on Codex or Claude Code with adaptive developer teams, Beads or local-file tracking, compact agent memory, named worktree teams, non-disruptive status, pause/resume recovery, preview approval, and authorized releases. Use for end-to-end development tasks requiring this harness, coordinated implementation streams, continued work already tracked through this harness, or agent-team start, status, pause, resume, and approve actions.
---
...
Read [session actions](references/actions.md) before setup or dispatch. Accept `$agent-team` in Codex, `/agent-team` in Claude Code, and clear plain-language equivalents. `start` without a name claims the next ready unassigned task and creates one named team with a stable team number. `resume` without a target recovers all unfinished teams in the current project, preserving gates and avoiding duplicate writers. Names/IDs select one team when supplied. `start [name] with-preview` requires explicit user approval before integration; `approve <name-or-ID>` approves only the submitted version.
