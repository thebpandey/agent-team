...
Existing v7 projects and hosts use the explicit native transaction `agent-teamctl cutover --request /absolute/path.json`. The v7 state and operation receipts are writable integrity records, not an immutable authorization root, so project cutover accepts only a short-lived Ed25519 approval whose signer is pinned outside the project. On Unix the fixed trust store is `/etc/agent-team/cutover-trust.json`; it must be root-owned, non-group/world-writable, schema 1, and contain a key-ID-sorted `keys` array. Windows currently fails closed because no native ACL verifier is provided. There is no v7-receipt, project-local, unsigned, or request-pinned fallback. The signed record binds project, operation, exact revision and task scope, canonical review/test/readiness IDs, paths and SHA-256 digests, recovery, and the configured remote URL/refs. The transaction observes that remote independently before mutation and again before receipt publication. The first project pass remains held; `reconcile` and rollback must match the recorded approval and receipt digest. Host requests are `host-cutover`, `host-status`, or `host-rollback`; they require the exact schema-4 v7 install receipt, including receipt ownership of top-level `SKILL.md`, and preserve a durable rollback receipt. Unknown, modified, or ambiguous v7 files and handlers are retained and fail closed.
