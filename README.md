...
> **Version 7.2.2.** This patch lets an adopted native owner reconcile an existing run and extend its tracked scope through exact installed-CLI commands. The `PreToolUse` adapter derives host, session, canonical working directory, and ownership epoch from the native event; caller-supplied identity at the exported CLI router is refused for these commands.
...
Agent-Team 7.2.2 accepts Project Kickoff 0.4.1 handoffs tested against 7.2.1 and retains the approved 0.4.1/7.2.0 compatibility pair.
...
Download `agent-team-7.2.2.zip` and its matching one-entry `SHA256SUMS` from the [Agent-Team v7.2.2 candidate release](https://github.com/thebpandey/agent-team/releases/tag/v7.2.2), or find updates through [releases/latest](https://github.com/thebpandey/agent-team/releases/latest). Verify the exact downloaded pair, then run the CLI from the extracted archive:

```bash
(cd /absolute/download && sha256sum -c SHA256SUMS)
node /absolute/extracted/agent-team/hooks/agent-team-cli.mjs install \
  --archive /absolute/download/agent-team-7.2.2.zip \
...
node hooks/agent-team-cli.mjs check-artifacts --revision <full-commit-id> --archive ../agent-team-artifacts/agent-team-7.2.2.zip
sha256sum ../agent-team-artifacts/agent-team-7.2.2.zip
```
