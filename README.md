...
Existing v7 projects and hosts use the explicit native transaction `agent-teamctl cutover --request /absolute/path.json`. A project request references a fresh canonical v7 `gate-evidence` integration operation by `approvalOperationId`; the transaction derives scope, cause, recovery, and remote expectations from that immutable evidence anchor and observes the configured remote again itself. It also binds the closed Beads export, exact Git revision, independently authored CLEAN review, and test/readiness digests. The first project pass remains held; a separate `reconcile` request must match the recorded approval operation and receipt digest. Host requests are `host-cutover`, `host-status`, or `host-rollback`; they require the exact schema-4 v7 install receipt, including receipt ownership of top-level `SKILL.md`, and preserve a durable rollback receipt. Unknown, modified, or ambiguous v7 files and handlers are retained and fail closed.
