...
Existing v7 projects and hosts use the explicit native transaction `agent-teamctl cutover --request /absolute/path.json`. A project request references either a fresh canonical v7 `gate-evidence` integration operation by `approvalOperationId`, when that gate covers the exact tracker scope, or a short-lived Ed25519 approval file by `approval: {path, sha256}`. The signed route is required for a stale Markdown project whose v7 gate cannot represent the target Beads scope; its public key and SHA-256 key ID must already be pinned as `cutoverApproval` in the receipt-owned v7 `setup.json`. There is no unsigned or request-pinned fallback. The transaction derives scope, cause, recovery, and remote expectations from the verified record and observes the configured remote again itself. It also binds the closed Beads export, exact Git revision, independently authored CLEAN review, and test/readiness digests. The first project pass remains held; a separate `reconcile` request must match the recorded approval and receipt digest. Host requests are `host-cutover`, `host-status`, or `host-rollback`; they require the exact schema-4 v7 install receipt, including receipt ownership of top-level `SKILL.md`, and preserve a durable rollback receipt. Unknown, modified, or ambiguous v7 files and handlers are retained and fail closed.
