...
> **Version 7.2.5.** This hotfix restores the trusted native path for evidence-store registration and published-history reconciliation. It observes one exact remote main ref through a bounded Git query and fails closed if that observation is missing, changed, malformed, or ambiguous. The 7.2.4 deployment-queue correction and explicit Project Kickoff compatibility allowlist are preserved.
...
Download `agent-team-7.2.5.zip` and its matching one-entry `SHA256SUMS` from the [Agent-Team v7.2.5 candidate release](https://github.com/thebpandey/agent-team/releases/tag/v7.2.5), or find updates through [releases/latest](https://github.com/thebpandey/agent-team/releases/latest). Verify the exact downloaded pair, then run the CLI from the extracted archive:

```bash
(cd /absolute/download && sha256sum -c SHA256SUMS)
node /absolute/extracted/agent-team/hooks/agent-team-cli.mjs install \
  --archive /absolute/download/agent-team-7.2.5.zip \
...
node hooks/agent-team-cli.mjs check-artifacts --revision <full-commit-id> --archive ../agent-team-artifacts/agent-team-7.2.5.zip
sha256sum ../agent-team-artifacts/agent-team-7.2.5.zip
```
