...
The hooks use Node.js 24 standard-library modules only. The transactional installer preserves unrelated host settings, keeps one authoritative Codex skill at `~/.agents/skills/agent-team`, and manages unchanged current Claude role definitions under `~/.claude/agents` without overwriting customized files. Run installation only with the user's authority. Health reports installation, registration, trust, runtime support, and observed exercise separately. Project health also reports the mapping cache used for fallback enforcement. Existing project owners can create that cache with the documented `migrate-mappings` command.
