...
Add a separate closed `project-owner-recover` transition for an initialized project whose historical native owner has stopped. It is exposed only through a Codex or Claude Code host integration that supplies an authenticated opaque capability bound to the native invocation, replacement host/session/CWD/writer identity, project, and expiry. The capability must be unforgeable and unmintable by an ordinary shell process, direct package import, unsigned hook event, environment variable, request file, or model-authored callback. A JavaScript object that merely has the documented context fields, an `assurance` string, liveness callback, and confirmation callback is not native authority.

The capability-authenticated host context, not the request or environment, supplies the replacement identity, a host-issued user approval bound to the project and old/new identities, and an authoritative old-session liveness observation. The request contains only bounded operation identity, expected versions/fingerprints/owner generation, project identity, and a human reason; it cannot assert an actor, replacement owner, host, approval, writer, or liveness result. Missing, expired, replayed, wrong-project, or caller-constructed capability context returns `native_owner_recovery_required` before liveness/confirmation callbacks, lock acquisition, journal creation, or any canonical write. That refusal preserves the exact bytes of setup, state, TEAMS, and owner history.
...
The audited Codex and Claude Code runtime surfaces do not currently provide a proven opaque recovery capability. Agent-Team 7.2.0 therefore may ship the closed schema, journal machinery, reader barrier, and fixture-only transition tests, but `project-owner-recover` remains unavailable in those live hosts and returns `native_owner_recovery_required` even when a caller constructs the complete documented object shape. A positive live recovery is not a 7.2.0 release or BrainVault acceptance claim. The stopped-owner success path is exercised only through a non-exported test capability factory until a host integration can issue and validate the real capability.
...
The artifact installer is qualified only on Linux and WSL with functioning `/proc/self/fd` traversal beneath an opened directory descriptor. It performs that read-only capability check before taking the install lock or making any mutation; another platform, a missing descriptor root, or a descriptor root that cannot traverse a child returns `unsupported_platform`. Every write, copy, and verification beneath a reserved package target resolves through the held directory-descriptor root, never through the replaceable canonical target pathname.

All package files, Claude role files, and hook declarations for the selected `codex`, `claude-code`, or explicit `both` host and `user` or project scope come only from that one sealed staging snapshot. Before any target, role, configuration, backup, or receipt mutation, the automatic installer preflights every selected package target. It may publish only to an absent target, or return unchanged when a present target's complete byte/mode/size map is exactly identical to the verified artifact. Any differing present target—including one recorded as owned or by a legacy/schema-3 receipt—returns `update_requires_manual_replacement` with `changed: false` and makes zero target, role, configuration, backup, or receipt mutations. This removes the impossible claim that a pure-Node automatic installer can both atomically replace an owned directory and avoid touching an uncoordinated operator replacement in the final pathname race.

Fresh absent-target publication remains one recoverable transaction across an explicit `both` selection: the installer revalidates staging before each publication, rehashes every installed target afterward, and retains staging until targets, configuration, and the receipt commit. If another actor renames or replaces the canonical name after reservation, descriptor-root writes remain confined to the invocation-owned directory and never pollute the operator's replacement. The final name/descriptor identity and exact-map check then blocks role/configuration and receipt publication. The installer cleans the invocation-owned descriptor tree as safely as its still-open identity permits; if another actor moved that namespace and exact removal cannot be proven, it retains durable recovery evidence rather than deleting an uncertain path. Archive, staged-tree, and installed-tree digests must agree; partial fresh publication rolls back from its journal. A later differing-version release requires an explicit quiesced maintenance procedure outside the automatic installer: validate and retain the current receipt, stop readers/writers, move the old target to a rollback backup, then invoke the normal fresh absent-target install and restore the backup if that install does not commit. Customized, unowned, ambiguous, and concurrently appeared resources remain preserved rather than overwritten. Optional Project Kickoff project hooks remain inactive unless separately approved.
...
- `hooks/lib/initialization.mjs` creates generation-1 ownership; `hooks/lib/workflow-cli.mjs` and `hooks/agent-team-cli.mjs` expose `project-owner-recover` only when the native adapter supplies a successfully authenticated opaque host capability. Shape-compatible caller objects never satisfy this condition. `hooks/lib/settings.mjs`, `hooks/lib/task-transitions.mjs`, and `hooks/lib/policy.mjs` compare runtime-qualified identity and refuse mixed/in-progress ownership generations.
...
- `hooks/lib/artifacts.mjs::verifyReleaseArtifact` owns the one-read archive authority object; `hooks/lib/install.mjs::installPackage` owns private archive-derived staging, the all-target absent/identical preflight, fresh-target transactions, rehashing, receipt schema migration, and rollback. It never performs an automatic changed-target replacement. No downstream copy helper accepts the moving source checkout for release installation.
- Focused tests are `tests/hooks-owner-recovery.test.mjs`, `tests/hooks-initialization.test.mjs`, `tests/hooks-project-journey.test.mjs`, `tests/hooks-canonical-state.test.mjs`, `tests/hooks-transitions.test.mjs`, `tests/hooks-policy.test.mjs`, `tests/hooks-artifacts.test.mjs`, `tests/hooks-install-health.test.mjs`, and `tests/hooks-package.test.mjs`. Owner-recovery tests directly import the package and pass a complete forged context, callbacks, and assurance string; the result is `native_owner_recovery_required` with byte-identical setup/state/TEAMS/history and no journal. BrainVault live-state acceptance is read-only; stopped-owner journal mechanics, legacy-run, `6zf`+`0o4`/`n8s`, evidence confinement, crash/replay, ancestry, and archive-tamper behavior use immutable fixtures and a non-exported test capability.
...
- `project-owner-recover` rejects shell/request/environment identity and a full-shape forged imported context with `native_owner_recovery_required` before callbacks or writes. Wrong native CWD/project, absent or mismatched host approval, active owner with `owner_active`, and unknown liveness are then tested behind the non-exported capability seam; every refusal preserves setup, state, TEAMS, and owner-history bytes and creates no journal.
- Fixture-only positive stopped recovery plus fresh native approval publishes exactly one runtime-qualified ownership generation across setup/state/TEAMS/owner-history; crash injection at each of the four record renames is journal-repaired, exact replay is duplicate, changed replay conflicts, and Codex/Claude identical session strings do not alias. Live 7.2.0 host acceptance makes no positive recovery claim while the opaque capability is unavailable.
...
- While BrainVault's observed owner remains active, live migration acceptance establishes that fact read-only, verifies that imported/full-shape recovery calls fail `native_owner_recovery_required`, and uses fixtures for the later `owner_active` and stopped-owner branches. It makes no takeover or canonical mutation and never stops the active teams to manufacture eligibility.
...
5. Re-read BrainVault setup/state/TEAMS/owner-history, native owner liveness, tracker, singleton, evidence-store declaration, Git ancestry, and publication target immediately before migration. The approved snapshot's current owner is active, so perform no takeover or live state mutation and do not stop its teams. Because current Codex and Claude Code hosts cannot provide the authenticated opaque capability, 7.2.0 live acceptance proves `native_owner_recovery_required` for direct/imported full-shape attempts and does not claim a successful recovery even if the owner later stops; `owner_active`, stopped-owner, four-record journal, history reconciliation, and settings-Cancel behavior are qualified with BrainVault-shaped fixtures. Live migration remains deferred until a separately verified host integration can issue the capability, the owner is naturally quiescent and positively proven stopped, and the unresolved `qac` writer/operation is reconciled. At that later boundary, register the declared evidence store using the exact `evidence-store-register` command, reconcile every snapshot-discovered missing historical delivery (`6zf` and `0o4` in the current snapshot) while preserving the observed active singleton (`n8s` currently), and populate the legacy run from explicit owner decisions. Every request uses freshly observed versions/fingerprints; no fixed singleton, version, task-only sequence, or HEAD equality is authoritative.
