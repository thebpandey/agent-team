...
| Last session's operational handoff | `.agent-team/SESSION.md` | A breadcrumb, not ownership, task, or worker authority. |
...
Pause stops assignment immediately. The orchestrator asks active agents to reach a safe checkpoint or uses the host's actual stop facility when requested, then writes `.agent-team/SESSION.md`: host, time, active Beads IDs, worktree/branch/revision, last observed native handles, outstanding blockers, and the next explicit action. It does not mark unfinished work complete. Resume rechecks Beads, Git, worktrees, and host-visible handles; it follows up with the same worker when that handle is available.
