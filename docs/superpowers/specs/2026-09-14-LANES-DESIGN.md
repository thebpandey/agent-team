...
The owner explicitly disabled the blocking installed PreToolUse handler. Immediate coordinator reassignment therefore uses an auditable user-directed-maintenance transaction, not a fabricated native hook observation. Its closed envelope binds project/root, old and requested new coordinator, epoch, setup/state versions, tracker identity/fingerprint, record fingerprints, operation ID, and authorization source. It runs under existing record locks with recoverable before-images, preserves claims and historical evidence, and invalidates old integration/release authority. It records liveness as not_asserted. This maintenance intent is not authentication and grants no release authority; ordinary lane/gate routes still require their native execution identity. Fresh sessions can edit unclaimed project files without the historical UUID, while registered worker boundaries and shared tracker/state restrictions remain enforced.
...
Independent per-host execution defaults: lanes.enabled=true; lanes.rotation.tasks=2; lanes.rotation.onPressure=true; lanes.factSheetStaleDays=7; lanes.workerUpdateMaxChars=2000; lanes.briefMaxWords=6000; supervision.heartbeatSeconds=600, minimum 60. Snapshot effective run settings. Safety limits: limits.subprocessMaxBufferBytes=2097152; limits.maxPlanTasks=1000; limits.canonicalRecordMaxBytes=16777216. The separate bounded canonical-record limit addresses the independently reproduced 1000-task lane state exceeding the previous 1 MiB reader cap. It does not increase worker-update or subprocess-output limits. Ensure downstream readers and initialization settings do not defeat supported limits.
...
Current executable hook payloads do not expose full session MCP/plugin visibility. Where genuine native observations are unavailable, a closed owner-reported proposal is explicitly offered_unverified with visibility unknown. Applying it requires native coordinator identity, exact reviewed selection, and explicit acknowledgement of unknown visibility. Filter required capabilities using the canonical dependency profile. Preserve owner_reported_unverified provenance in receipts, replay, and health; never present it as native observation. Derive Claude home internally from trusted adapter context, not request data. Automatic parent-model comparison likewise remains unknown without trustworthy comparable metadata.
