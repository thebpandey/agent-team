...
func render(s Snapshot) ([]byte, error) {
	...
	for _, task := range s.Tasks { out.WriteString("<tr><td>"); escaped(&out, task.ID); out.WriteString("</td><td>"); escaped(&out, task.State); out.WriteString("</td><td>"); escaped(&out, task.Next); out.WriteString("</td></tr>") }
	out.WriteString("</tbody></table></section><section><h2>Teams</h2>")
	for _, team := range s.Teams { tag(&out, "h3", team.ID+" · "+team.State); tag(&out, "p", "Queue: "+team.QueueFingerprint); list(&out, team.Tasks) }
	out.WriteString("</section><section><h2>Resources</h2>")
	listSection(&out, "Servers", s.Resources.Servers); listSection(&out, "Browsers", s.Resources.Browsers); listSection(&out, "External", s.Resources.External)
...
func tag(out *strings.Builder, name, value string) { out.WriteByte('<'); out.WriteString(name); out.WriteByte('>'); escaped(out, value); out.WriteString("</"); out.WriteString(name); out.WriteByte('>') }
func escaped(out *strings.Builder, value string) { out.WriteString(html.EscapeString(value)) }
func listSection(out *strings.Builder, name string, values []string) { tag(out, "h3", name); list(out, values) }
func list(out *strings.Builder, values []string) { out.WriteString("<ul>"); for _, value := range values { tag(out, "li", value) }; out.WriteString("</ul>") }
